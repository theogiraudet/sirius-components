...
public class ELKLayoutedDiagramProvider {
    ...
    private Label getNodeLayoutedLabel(Node node, Map<String, ElkGraphElement> id2ElkGraphElements, double xOffset, double yOffset, ISiriusWebLayoutConfigurator layoutConfigurator) {
        ...
        if (optionalElkBeginLabel.isPresent()) {
            ...
            Position position = Position.at(xOffset + elkLabel.getX(), yOffset + elkLabel.getY());
            Size nodeSize = Optional.of(elkLabel.getParent()).filter(ElkNode.class::isInstance).map(ElkNode.class::cast).map(elkNode -> {
            ...
            for (ICustomNodeLabelPositionProvider customLabelPositionProvider : this.customLabelPositionProviders) {
                Optional<Position> positionOpt = customLabelPositionProvider.getLabelPosition(layoutConfigurator, size, nodeSize, node.getType(), node.getStyle());
