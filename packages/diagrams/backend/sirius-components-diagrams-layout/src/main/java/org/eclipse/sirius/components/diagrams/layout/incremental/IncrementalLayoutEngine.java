...
public class IncrementalLayoutEngine {
    ...
    private void layoutNode(Optional<IDiagramEvent> optionalDiagramElementEvent, NodeLayoutData node, ISiriusWebLayoutConfigurator layoutConfigurator) {
        ...
        if (node.getLabel() != null) {
            Position nodeLabelPosition = this.nodeLabelPositionProvider.getPosition(node, node.getLabel(), borderNodesOnSide);
            // The label position may be customized
            for (ICustomNodeLabelPositionProvider customLabelPositionProvider : this.customLabelPositionProviders) {
                Optional<Position> nodeLabelPositionOpt = customLabelPositionProvider.getLabelPosition(layoutConfigurator, node.getLabel().getTextBounds().getSize(), node.getSize(),
                        node.getNodeType(), node.getStyle());
                if (nodeLabelPositionOpt.isPresent()) {
                    nodeLabelPosition = nodeLabelPositionOpt.get();
                    break;
                }
            }
