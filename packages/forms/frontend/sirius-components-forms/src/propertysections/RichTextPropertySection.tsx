...
export const RichTextPropertySection = ({
  ...
  const [{ value: schemaValue, context }, dispatch] = useMachine<
    ...
    RichTextPropertySectionEvent
  >(RichTextPropertySectionMachine);
  const { richTextPropertySection, toast } = schemaValue as SchemaValue;
  ...
  const sendEditedValue = (newValue) => {
    // if (richTextPropertySection === 'edited') {
  ...
  const onSave = (newValue: string) => {
    console.log('onSave(' + JSON.stringify(newValue) + ')');
  ...
  return (
    <div>
      ...
      <RichTextEditor
        value={value}
        pristine={richTextPropertySection === 'pristine'}
